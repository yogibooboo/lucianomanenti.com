...
console.log(`Artwork Group parent matrix translation: translate(3.5433 212.6)`);
console.log(`\nListing first 10 paths in artwork group:`);
artworkGroup.children.slice(0, 10).forEach((child, idx) => {
    if (child.name === 'path') {
        console.log(`Path ${idx}: d="${child.attrs.d.substring(0, 80)}..."`);
    } else {
        console.log(`Child ${idx}: <${child.name}>`);
    }
});
