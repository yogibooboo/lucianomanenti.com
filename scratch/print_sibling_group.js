...
// Parent group of the <g> that contains targetRect:
// targetRect is in <g> (inner rect scale), which is in <g> (outer transform), which is in <g> (card container group)
const cardContainerGroup = targetRect.parent.parent.parent;
console.log(`Card Container Group Name: <${cardContainerGroup.name}>`);

// Print all children of cardContainerGroup
cardContainerGroup.children.forEach((child, idx) => {
    ...
    // Print deep children summary
    const summary = [];
    function summarize(n) {
        summary.push(n.name);
        if (n.children) n.children.forEach(summarize);
    }
    child.children.forEach(summarize);
    console.log(`  Contains elements: ${summary.slice(0, 10).join(', ')}${summary.length > 10 ? '...' : ''}`);
});
