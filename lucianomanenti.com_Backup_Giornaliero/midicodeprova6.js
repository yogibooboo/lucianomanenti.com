...
$('.pulsanteaiuto').click(function() {/*	window.open("instructions/hanon_test.htm", "_blank", "toolbar=no, scrollbars=yes, resizable=yes, top=100, left=400, width=1000, height=800");
	//window.open("instructions/hanon_test.htm") */ Jazz.MidiOut();
});
...
function initDevices(midi) {
    ...
    if (midi.inputs.size != 0) sel.length=0;

    
    const inputs = midi.inputs.values();
    for (let input = inputs.next(); input && !input.done; input = inputs.next()) {
        midiIn.push(input.value);
        sel[sel.options.length]=new Option(input.value.name);
    }

    // MIDI devices that you send data to.

    sel=document.getElementById("midiOut");
    if (midi.outputs.size != 0) sel.length=0;


    const outputs = midi.outputs.values();
    for (let output = outputs.next(); output && !output.done; output = outputs.next()) {
        midiOut.push(output.value);
        sel[sel.options.length]=new Option(output.value.name);
    }
...
function midiMessageReceived(event) {
    ...
    midiProc(0,event.data[0],event.data[1],event.data[2]);
...
var Jazz = {
    ...
    MidiOut: function(comando,nota,velocita) {
  const NOTE_ON = 0x90;
  const NOTE_OFF = 0x80;
  
  const device = midiOut[0];
  const msgOn = [comando,nota,velocita];
  //const msgOff = [NOTE_OFF, nota, velocita];
  ...
  // Then send the note off. You can send this separately if you want 
  // (i.e. when the button is released)
  //device.send(msgOff, Date.now() + 100); 
        return;
    },
...
function midiProc(t, a, b, c) {
    ...
    if ((a == 0x90) && (b == 27))
        return (tmidi.setbpm(tmidi.bpm + 5))
    //terzo tasto nero
    if ((a == 0x90) && (b == 106) && (tmidi.hanonselected < 19)) {
        tmidi.hanonselected++;
        tmidi.loadfiles()
    }
    ;if ((a == 0x90) && (b == 104) && (tmidi.hanonselected > 1)) {
        tmidi.hanonselected--;
        tmidi.loadfiles()
    }
    ...
    if (tmidi.fsuona && !tmidi.fintro) {
        ...
        if (a == 0x90) {
            ...
            gestnote = (function(sin, des, prima) {
                ...
                if ((des) && (b == tmidi.cbuffer[pospresunta])) {
                    //mano destra
                    ...
                    log("DX   pos= " + pospresunta + " nota on" + b + " nota teor" + tmidi.cbuffer[pospresunta] + " " + prima + " tempo=" + tempo + " inizio=" + tmidi.inizioinput + " delta=" + ritardo)
                    return true
                }
                if ((sin) && (b == (tmidi.cbuffer[pospresunta] - 12))) {
                    //mano sinistra
                    ...
                    log("SX   pos= " + pospresunta + " nota on" + b + " nota teor" + (tmidi.cbuffer[pospresunta] - 12) + " " + prima + " tempo=" + tempo + " inizio=" + tmidi.inizioinput + " delta=" + ritardo)
                    return true
                }
                return false
            ...
            if (gestnote(true, true, true))
                return;
            //gestisce sinistra e destra
            ...
            if (pospresunta > 0) {
                var sx = false
                  , dx = false;
                if (tmidi.barrasinistra.length < (pospresunta))
                    sx = true;
                if (tmidi.barradestra.length < (pospresunta))
                    dx = true;
                pospresunta--
                if (gestnote(sx, dx, false))
                    return;
                pospresunta++;
            }

            log("NULL pos= " + pospresunta + " nota on" + b + " nota teor" + tmidi.cbuffer[pospresunta] + " tempo=" + tempo + " inizio=" + tmidi.inizioinput + " delta=" + ritardo)
        ...
        if (a == 0x80) {
            ...
            if ((trovato && dx)) {
                var ritardo = tempo - tmidi.inizioinput - tmidi.intervallo * (pos + 1);
                //rispetto a inizio nota successiva

                tmidi.deltastopd[pos] = ritardo;
                log("DXOFF " + (pos) + " notain:");
                while (tmidi.barradestra.length < (pos)) {
                    tmidi.barradestra.push({
                        "s": 0,
                        "w": 200,
                        "c": "red",
                        "e": true,
                    ...
                    tmidi.colorenotadestra[tmidi.barradestra.length - 1] = "#FF0000"
                    //red
                }
                ...
                tmidi.colorenotadestra[pos] = colore
                //blue
                return
            }
            if ((trovato && !dx)) {
                ...
                while (tmidi.barrasinistra.length < (pos)) {
                    ...
                    tmidi.colorenotasinistra[tmidi.barrasinistra.length - 1] = "#FF0000"
                    //red
                }
...
var tmidi = {
    ...
    onPlayer: function(e) {
        ...
        if (e.control == 'play') {//btn.innerHTML='stop';
    ...
    initoptions: function(b, w, h) {
        ...
        for (var i = 0; i < nsw; i++) {
            if (tmidi.opzioni[i])
                b.fillRect(w - 50, 10 + i * 25, 40, 25);
            else
                b.fillRect(w - 90, 10 + i * 25, 40, 25);
        }
    ...
    initgraficoscroll: function(b, w, h) {
        ...
        var lnota = 50;
        //lunghezza in pixel di una nota  (1/16 bpm)
        var buf = tmidi.BufferNote;
        //contiene le note della mano destra del brano  (n. tasto biancocon 0 a partire da do sotto due ottave)
        var cbuf = tmidi.cbuffer;
        //contiene le note midi della mano destra (36 è il do sotto due ottave)
        ...
        var asse = 0;
        //diventerà relativo a performance.now(), espresso in numero di sedicesimi
        var bufn = tmidi.notesuonate;

        asse = -tmidi.eoffset / 16

        b.fillStyle = "#FFFFFF";
        b.fillRect(700 - 80, 0, 80, ntasti * ltasti);
        //rettangolo bianco tastiera
        ...
        for (var i = 0; i < bufn.length; i++) {
            bn = bufn[i];
            inizio = iniziale;
            fine = asse * tmidi.sedicesimo;
            if (bn.tstart > finale)
                continue;
            if (bn.tstart > iniziale)
                inizio = bn.tstart;
            if (bn.tstop != 0) {
                if (bn.tstop < iniziale)
                    continue;
                if (bn.tstop < finale)
                    fine = bn.tstop;
                else
        ...
        for (var i = Math.floor(Math.max(asse - 700 / lnota, 0)); i < Math.min(Math.ceil(asse + 500 / lnota), buf.length); i++) {
            ...
            if (i == Math.floor(asse)) {
                b.fillStyle = "#AAAAFF";
                b.fillRect(700 - 80, nota * ltasti, 80, ltasti);
                //tasto azzurro
                b.fillStyle = "#AAFFAA";
                b.fillRect(700 - 80, (nota - 7) * ltasti, 80, ltasti);
                //tasto verde
            }
            //b.fillStyle="#0000FF";
    ...
    initgraficosp: function(b, w, h) {
        ...
        var cnd = tmidi.colorenotadestra
          , cns = tmidi.colorenotasinistra;

        if ((tmidi.fsuona) && !(tmidi.fintro))
            zinizio = tmidi.inizioinput;
        //zeppa nel caso di prima inizializzazzione
        ...
        if (esamina)
            zoffset = -tmidi.eoffset;

        var d = 500, posy, posx = 0, nota, oldnota, oldposx, oldposy;
        var cols, cold;
        var td = tmidi.barradestra
          , ts = tmidi.barrasinistra;
        ...
        for (var i = 0; ((i < lung) && (posx < limite)); i++) {
            ...
            if (posx > 100) {
                ...
                b.fill();
                if ((nota == 9) || (nota == 21) || (nota == 7))
                    drawline(posy + 28);
                if ((nota == 7) || (nota == 8))
                    b.fillRect(posx - 6, 76 + 28, 10, 1);
                ...
                if ((i % 4) == 3) {
                    ...
                    b.fillRect(oldposx + 3, oldposy, 1, -25);
                    //prima stanghetta verticale del gruppo di 4
                    b.fillRect(oldposx - 4, oldposy + 28, 1, 25);

                    var oldposx2, oldposy2, delta2;
                    //seconda stanghetta verticale del gruppo di 4
                    ...
                    b.fillRect(posx + 3, posy, 1, -25);
                    //Ultima stanghetta verticale del gruppo di 4
    ...
    refresh: function() {
        ...
        if (tmidi.fesamina) {
            if (tmidi.fsuona)
                tmidi.eoffset = -(performance.now() - tmidi.inizio) / tmidi.intervallo * 16 + tmidi.Bintro.length * 64;
    ...
    initnotecolori: function() {
        for (i = 0; i < tmidi.cbuffer.length; i++) {
            tmidi.colorenotadestra[i] = "#000000";
            //nero
            tmidi.colorenotasinistra[i] = "#000000";
            //nero
    ...
    inizializzazioni: function() {
        ...
        //Jazz.MidiOut(0x90, 60, 0);
        //sembra che il MidiOut si debba inizializzare...
        ...
        tmidi.veld = [];
        tmidi.barrasinistra = [];
        //{"s":start,"w":width,"c":color,"e":error}
        tmidi.barradestra = [];
        ...
        tmidi.latenza = 10;
        tmidi.BufferNote = [];
        //note con 0 a partire da do sotto due ottave
    ...
    aggiornaerrori: function() {
        ...
        bd = tmidi.barradestra;
        bs = tmidi.barrasinistra;
        lungd = bd.length;
        lungs = bs.length;
        lungt = lungd;
        if (lungs < lungt)
            lungt = lungs;
        //lungt è il minore dei due indici
        ...
        for (var i = 0; i < lungt; i++) {
            if (bd[i].e)
                ...
                sdeltastartd += Math.abs(tmidi.deltastartd[i]);
                //mano destra
            ...
            if (bs[i].e)
                ...
                sdeltastarts += Math.abs(tmidi.deltastarts[i]);
                //mano sinistra
        ...
        tmidi.errnoted = Math.floor(tmidi.numerrnoted / lungt * 100);
        //note errate in percentuale sulle eseguite
        ...
        for (var i = 0; i < lungt; i++) {
            if (!bd[i].e) {
                tmidi.erregad += Math.pow((tmidi.deltastartd[i] - smediastartd), 2);
            ...
            if (!bs[i].e) {
                tmidi.erregas += Math.pow((tmidi.deltastarts[i] - smediastarts), 2);
                //sommatoria dei quadrati
            ...
            if (!(bd[i].e) && !(bs[i].e)) {
                //buoni contemporanei
                tmidi.erregap += Math.pow(((tmidi.deltastartd[i] - tmidi.deltastarts[i]) - smediastartp), 2);
                //sommatoria dei quadrati
        ...
        if (notebuoned > 0) {
            tmidi.erregad = 100 - Math.floor(Math.sqrt(tmidi.erregad / notebuoned) / tmidi.intervallo * 100);
            //RMS
    ...
    setbpmx: function(ev) {
        ...
        if ((x < 108) && (y < 20))
            return tmidi.setbpm(tmidi.bpm + 1);
        if ((x < 108))
            return tmidi.setbpm(tmidi.bpm - 1);
        if ((x > 108) && (y < 20))
            return tmidi.setbpm(tmidi.bpm + 5);
    ...
    initbuffernote: function() {
        ...
        for (var i = 0; i < tmidi.cbuffer.length; i++) {
            tmidi.BufferNote[i] = tmidi.convertinote(tmidi.cbuffer[i]);
            //note con 0 a partire da do sotto due ottave
        }
    },
    convertinote: function(nota) {
        //converte nota da numerazione midi a numerazione tasti bianchi con 0 a partire da do sotto due ottave
        nota -= 36;
    ...
    startstop: function(opzione) {
        ...
        if (opzione == "specialintro")
            tmidi.drumsintro[2] = 78;
        if (tmidi.fsuona) {
            tmidi.stopsuona();
            return
        }
        ...
        tmidi.inizio = performance.now() + 100
        //+1000;  //comincerà fra un secondo
        tmidi.next = tmidi.inizio;
        tmidi.inizioinput = tmidi.inizio + 100
        //+1000;
        tmidi.notacorrente = 0;
        //tmidi.sinistracorrente=-1;
        //tmidi.sinistranota=[];
    ...
    startnota: function() {
        ...
        if (tmidi.fintro) {
            ...
            setTimeout(tmidi.stopnotaintro, tmidi.next + duratanota - performance.now(), nota);
            //richiede nota off
            ...
            if (tmidi.notacorrente >= tmidi.Bintro.length) {
                ...
                }, tmidi.next - performance.now());
                tmidi.notacorrente = 0;
                tmidi.inizioinput = tmidi.next;
            }
            setTimeout(tmidi.stopnotaintro, tmidi.next - tmidi.intervallo + duratanota - performance.now(), nota);
            //richiede nota off
            return
        }
        //if (tmidi.fintro) 
        ...
        if (tmidi.notacorrente % 4 == 0) {
            ...
            setTimeout(tmidi.stopnotaintro, tmidi.next + duratanota - performance.now(), 75);
            //richiede nota off
        ...
        if (tmidi.notacorrente >= tmidi.cbuffer.length - 1) {
            duratanota = tmidi.intervallo * 3 + tmidi.durata;
            //ultima nota lunga
        ...
        setTimeout(tmidi.stopnota, tmidi.next - tmidi.intervallo + duratanota - performance.now(), nota);
        //richiede nota off
        tmidi.aggiornaerrori();
    },
    ...
    stopnota: function(nota) {
        ...
        if (tmidi.fcancellasuona) {
            ...
            var opzintro = "";
            if ((tmidi.opzioni[3]) && (tmidi.oldtotale >= 70)) {
                //auto bpm+5 se punteggio >70

                Jazz.MidiOut(0x99, 78, 120);
                setTimeout(tmidi.stopnotaintro, tmidi.next + tmidi.intervallo - performance.now(), 78);
                //richiede nota off
    ...
    creacontatore: function(nome, larghezza, altezza, contenitore, posx, posy, opzione) {
        ...
        var wdigit = Math.floor(larghezza * 0.9 / 3)
          , hdigit = Math.floor(altezza * 9 / 10);
        var offsetx = Math.round(1 + larghezza / 50);
        var offsety = Math.round(1 + altezza / 50);
        var largopzione = 0;
        ...
        $("#" + contenitore).append('<div id="' + nome + '" style="top: ' + posy + 'px; left: ' + posx + 'px;" class="contatore">' + '<img src=' + immagine + ' height="' + altezza + 'px" width="' + (larghezza + largopzione) + 'px">' + '<div id="digit3" class="digitx" style="top: ' + offsety + 'px; left: ' + offsetx + 'px; width:' + wdigit + 'px; height:' + hdigit + 'px;' + '	background-size: ' + wdigit + 'px ' + (hdigit * 10) + 'px; background-position: -0px 0px; "  > </div>' + '<div id="digit2" class="digitx" style="top: ' + offsety + 'px; left: ' + Math.floor(offsetx + larghezza / 3.2) + 'px; width:' + wdigit + 'px; height:' + hdigit + 'px;' + '	background-size: ' + wdigit + 'px ' + (hdigit * 10) + 'px; background-position: -0px 0px; "  > </div>' + '<div id="digit1" class="digitx" style="top: ' + offsety + 'px; left: ' + Math.floor(offsetx + larghezza / 3.2 * 2) + 'px; width:' + wdigit + 'px; height:' + hdigit + 'px;' + '	background-size: ' + wdigit + 'px ' + (hdigit * 10) + 'px; background-position: -0px 0px; "  > </div> </div>')
    ...
    displaypunti: function(punti, display) {
        ...
        if (punti > 999)
            punti = 999;
        centinaia = Math.floor(punti / 100);
        punti -= (centinaia * 100);
        decine = Math.floor(punti / 10);
        punti -= (decine * 10);
        unita = punti;
        ...
        $("#" + display + " #digit3").animate({
            "pippocentinaia": ((-altezza * centinaia) + "px")
        }, {
        ...
        $("#" + display + " #digit2").animate({
            "pippodecine": ((-altezza * decine) + "px")
        }, {
            step: function(now, fx) {
                $("#" + display + " #digit2").css({
                    "background-position": ("0px " + now + "px")
        ...
        $("#" + display + " #digit1").animate({
            "pippounita": ((-altezza * unita) + "px")
        }, {
            step: function(now, fx) {
                $("#" + display + " #digit1").css({
                    "background-position": ("0px " + now + "px")
    ...
    displaypuntifast: function(punti, display) {
        ...
        $("#" + display + " #digit3").css({
            "background-position": ("0px " + (-altezza * centinaia) + "px")
        });
        $("#" + display + " #digit2").css({
            "background-position": ("0px " + (-altezza * decine) + "px")
        });
        $("#" + display + " #digit1").css({
            "background-position": ("0px " + (-altezza * unita) + "px")
        });
    },
